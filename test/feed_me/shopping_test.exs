defmodule FeedMe.ShoppingTest do
  ...
  describe "add_to_pantry feature" do
    ...
    test "checking pre-linked item on auto-add list syncs to pantry immediately",
         %{household: household, user: user} do
      location = FeedMe.Pantry.get_pantry_location(household.id)
      ...
      {:ok, list} =
        Shopping.create_list(%{
          ...
          add_to_pantry: true,
          auto_add_to_location_id: location.id
        })
      ...
      # Pre-linked item should be added immediately, NOT queued
      updated_pantry_item = FeedMe.Pantry.get_item(pantry_item.id)
      assert Decimal.equal?(updated_pantry_item.quantity, Decimal.new("5"))
    end

    test "unchecking pre-linked item reverses the immediate pantry sync",
         %{household: household, user: user} do
      location = FeedMe.Pantry.get_pantry_location(household.id)
      ...
      {:ok, item} = Shopping.add_from_pantry(list.id, pantry_item, Decimal.new("4"), user)

      # Check: immediately adds 4
      {:ok, checked} = Shopping.toggle_item_checked(item, user.id)
      assert Decimal.equal?(FeedMe.Pantry.get_item(pantry_item.id).quantity, Decimal.new("10"))

      # Uncheck: immediately reverses
      {:ok, _unchecked} = Shopping.toggle_item_checked(checked, user.id)
      assert Decimal.equal?(FeedMe.Pantry.get_item(pantry_item.id).quantity, Decimal.new("6"))
    end

    test "checking unlinked item on auto-add list queues for AI sync",
         %{household: household, user: user} do
      ...
      # Unlinked item should be queued for AI matching
      Process.sleep(10)
      assert FeedMe.Pantry.Sync.pending_count(household.id, location.id) == 1
    end

    test "checking item on non-auto-add list does NOT queue for sync",
         %{household: household, user: user} do
      Application.put_env(:feed_me, FeedMe.Pantry.Sync, debounce_ms: 60_000, enabled: true)
      ...
      Process.sleep(10)
      assert FeedMe.Pantry.Sync.pending_count(household.id, location.id) == 0

      updated_pantry_item = FeedMe.Pantry.get_item(pantry_item.id)
      assert Decimal.equal?(updated_pantry_item.quantity, Decimal.new("6"))
    end

    test "unchecking unlinked item dequeues it from sync",
         %{household: household, user: user} do
      Application.put_env(:feed_me, FeedMe.Pantry.Sync, debounce_ms: 60_000, enabled: true)
      ...
      # Check the item (queues it for AI)
      {:ok, checked} = Shopping.toggle_item_checked(item, user.id)
      assert checked.checked == true
      Process.sleep(10)
      assert FeedMe.Pantry.Sync.pending_count(household.id, location.id) == 1
